...
ILOMIPINFOCALLBACK4(timeIntervalCallback,
                    IloCplex, cplex,
                    ofstream &, fp,
                    IloNum, start_time,
...
void read_instance(const IloEnv &env, const std::string &file_name, IloInt &nodes_no, IloInt &vehicles_no, IloInt &horizon, IloNumArray &r, IloNumArray &d, IloNumArray &e, IloNumArray &l, IloNumArray &m, IloNumArray &s, IloNumArray2 &tt, IloNumArray &Q, IloNumArray &c)
{
    ...
    r = IloNumArray(env, nodes_no);
    d = IloNumArray(env, nodes_no);
    ...
    for (int i = 0; i < nodes_no; i++)
    {
        ...
        {
            tt[i][j] = int(euclidean_distance_calc(demands[i].x, demands[i].y, demands[j].x, demands[j].y) * 10) / 10.0;
        }
    }
...
int main(int argc, char **argv)
{
    ...
    try
    {
        IloModel model(env);
        // Variable Define
        BoolVar4Matrix x(env, horizon + 1);
        for (int t = 1; t <= horizon; t++)
        {
            x[t] = BoolVar3Matrix(env, vehicles_no);
            for (int k = 0; k < vehicles_no; k++)
            {
                x[t][k] = BoolVarMatrix(env, nodes_no);
                for (int i = 0; i < nodes_no; i++)
                {
                    x[t][k][i] = IloBoolVarArray(env, nodes_no);
                    for (int j = 0; j < nodes_no; j++)
                    {
                        x[t][k][i][j] = IloBoolVar(env, std::string("x_{" + std::to_string(i) + "," + std::to_string(j) + "," + std::to_string(k) + "}_" + std::to_string(t)).c_str());
                    }
                }
            }
        }
        ...
        for (int i = 0; i < nodes_no; i++)
        {
            ...
            at[i] = IloNumVar(env, 0, IloInfinity, IloNumVar::Float, std::string("at_" + std::to_string(i)).c_str());
            dt[i] = IloNumVar(env, 0, IloInfinity, IloNumVar::Float, std::string("dt_" + std::to_string(i)).c_str());
        }
        ...
        for (int t = 1; t <= horizon; t++)
        {
            for (int k = 0; k < vehicles_no; k++)
            {
                for (int i = 0; i < nodes_no; i++)
                {
                    for (int j = 0; j < nodes_no; j++)
                    {
                        traveling_cost += c[k] * tt[i][j] * x[t][k][i][j];
                    }
                }
            }
        }
        ...
        for (int i = 0; i < nodes_no; i++)
        {
            overtime_wages += IloMax(0, at[i] - T_max) * f;
        }
        model.add(IloMaximize(env, profits - traveling_cost - outsourcing_cost - overtime_wages));
        profits.end();
        traveling_cost.end();
        ...
        for (int k = 0; k < vehicles_no; k++)
        {
            for (int t = 1; t <= horizon; t++)
            {
                for (int i = 0; i < nodes_no; i++)
                {
                    for (int j = 0; j < nodes_no; j++)
                    {
                        model.add(x[t][k][i][j] >= 0);
                        model.add(x[t][k][i][j] <= 1);
                    }
                }
            }
        }
        ...
        //                 {
        //                     std::cout << i << " served by " << t << " " << k << " " << j << std::endl;
        //                 }
        //             }
        //         }
        //     }
